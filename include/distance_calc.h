...
std::vector<BlockInfo> processAndSendBlocks(std::vector<BlockInfo>& blockInfos, 
    ...
    const std::vector<std::pair<std::vector<double>, int>>& allCenters_test, 
    double distance_threshold, 
    const std::vector<int>& permutation, const std::vector<int>& localPermutation, 
    const Opts& opts, bool pred_tag);
