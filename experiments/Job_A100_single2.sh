    ...
    for M_est in ${M_ests[@]}; do
        for N_b in ${N_bs[@]}; do
            for i in {1..5}; do
                ...
                srun --exclusive  ./bin/dbv --num_total_points $N --num_total_blocks $bc -m $m_bv --dim $DIM --mode estimation --maxeval 350 --xtol_rel 1e-8 --ftol_rel 1e-8 --kernel_type Matern72 --seed $i --nn_multiplier 50 --log_append A100_single2 --omp_num_threads 10 --print=false
            done
        done
    done
done
